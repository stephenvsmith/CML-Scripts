...
library(CML,quietly=TRUE,verbose = FALSE,warn.conflicts = FALSE)
...
run_cml_target <- function(t,df,num,results_pc,algo,curr_dir){
  output_text(t,num,algo,"cml")
  setwd(curr_dir)
  if (file.exists(paste0("cml_",array_num,"_",num,"_",paste(t,collapse = "_"),"_results.rds"))){
    cat("Downloaded\n")
    return(readRDS(paste0("cml_",array_num,"_",num,"_",paste(t,collapse = "_"),"_results.rds")))
  }
  
  # Run CML
  results <- run_cml(t,df,num,results_pc,algo)
  saveRDS(results,paste0("cml_",array_num,"_",num,"_",paste(t,collapse = "_"),"_results.rds"))
  return(results)
}

run_snl_target <- function(t,df,num,results_pc,algo,curr_dir){
  output_text(t,num,algo,"snl")
  setwd(curr_dir)
  if (file.exists(paste0("snl_",array_num,"_",num,"_",paste(t,collapse = "_"),"_results.rds"))){
    cat("Downloaded\n")
    return(readRDS(paste0("snl_",array_num,"_",num,"_",paste(t,collapse = "_"),"_results.rds")))
  }
  # Run SNL
  results <- run_snl(t,df,num,results_pc,algo)
  
  saveRDS(results,paste0("snl_",array_num,"_",num,"_",paste(t,collapse = "_"),"_results.rds"))
  return(results)
}

# CML Helpers -------------------------------------------------------

run_cml <- function(t,df,num,results_pc,algo){
  # Set initial values for lmax and start timer
  val_lmax <- results_pc$lmax$PC
  lmax <- max(1,val_lmax)
  results_pc$lmax[["CML"]] <- lmax
  true_dag <- network_info$true_dag
  start <- Sys.time()
  # Run CML
  cml_result <- cml(data=df,
                              targets=t,
                              lmax=lmax,
                              tol=alpha,
                              mb_tol=mb_alpha,
  ...
  diff <- end - start
  units(diff) <- "secs"
  results_pc$time_diff[["CML"]] <- diff
  results_pc$num_tests[["CML"]] <- cml_result$NumTests
  check_anc_edge_markings(cml_result$amat,
                          paste0(result_dir,"/anc_edge_errors.txt"),
                          array_num)
  results <- neighborhood_results(t,cml_result,results_pc,num)
  return(results)
}

run_snl <- function(t,df,num,results_pc,algo){
  ...
  diff <- end - start
  units(diff) <- "secs"
  snl_result$time_diff <- diff
  snl_result$lmax<- lmax
  results <- neighborhood_results_pc(t,snl_result,num)
...
get_result_metrics <- function(targets,graphs,nbhds,method='cml'){
  if (method=='pc'){
  ...
  if (method=='cml') {
    ...
    results <- results %>% 
      mutate(pc_num_tests=pc_result$num_tests[["PC"]],
             cml_num_tests=pc_result$num_tests[["CML"]],
             pc_time=pc_result$time_diff[["PC"]],
             cml_time=pc_result$time_diff[["CML"]]) %>% 
      mutate(pc_lmax = pc_result$lmax$PC,
             cml_lmax=pc_result$lmax[["CML"]]) %>%
    ...
    ### Simplified results data for SNL
    results <- data.frame(
      snl_time=as.numeric(local_result$time_diff),
      snl_lmax=local_result$lmax,
      snl_num_tests=local_result$NumTests,
      snl_rule0=local_result$rules_used[1],
      snl_rule1=local_result$rules_used[2],
      snl_rule2=local_result$rules_used[3],
      snl_rule3=local_result$rules_used[4],
      snl_rule4=local_result$rules_used[5],
      targetSkeletonTimes=paste(local_result$targetSkeletonTimes,collapse = ","),
      totalcpptime=local_result$totalTime
    )
...
clean_modify_results <- function(results,num,t){
  results <- results %>% 
    ...
    select(sim_number,net,p,net_edges,trial_num,n,alpha,mb_alpha,algo,ub,high,
           ...
           contains("cml"),contains("mb",ignore.case=FALSE),contains("pc"),
...
# Compile all results about the simulation for CML
neighborhood_results <- function(t,cml_result,pc_results,num){
  # Obtain narrow and broad neighborhoods
  nbhd <- get_nbhds(t,cml_result)
  # Obtain narrow and broad matrices from algorithms and reference
  mats <- get_ref_mats(nbhd,pc_results,cml_result)
  ...
  # Overall F1 Score
  cml_metrics <- get_result_metrics(t,mats,nbhd)
  ...
  if (length(nbhd)>0){
    ...
    cml_metrics$cml_narrow_shd <- get_shd(mats$ground_narrow,mats$local_narrow_shd)
    cml_metrics$cml_broad_shd <- get_shd(mats$ground_broad,mats$local_broad_shd)
  } else {
    pc_metrics$pc_narrow_shd <- 0
    pc_metrics$pc_broad_shd <- 0
    cml_metrics$cml_narrow_shd <- 0
    cml_metrics$cml_broad_shd <- 0
  }
  
  
  add_results <- additional_metrics(mats,cml_result,pc_results,t,nbhd)
  
  # Combine results
  results <- cbind(cml_metrics,pc_metrics,add_results)
  results <- clean_modify_results(results,num,t)
  
  return(results)
}

neighborhood_results_pc <- function(t,snl_result,num){
  # Obtain narrow and broad neighborhoods
  nbhd <- get_nbhds(t,snl_result)
  # Obtain narrow and broad matrices from algorithms and reference
  mats <- get_ref_mats(nbhd,NULL,snl_result,get_pc = FALSE)

  snl_results <- get_result_metrics(t,mats,nbhd,method = 'snl')
  snl_results$snl_narrow_shd <- get_shd(mats$ground_narrow,mats$local_narrow)
  snl_results$snl_broad_shd <- get_shd(mats$ground_broad,mats$local_broad)
  add_results <- additional_metrics(mats,snl_result,pc_results,
                                    t,nbhd,method = 'snl') %>% 
    mutate(trial_num=num,
           targets=paste(t,collapse = ","))
  results <- cbind(snl_results,add_results)
